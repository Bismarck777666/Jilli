...
{
    class FG3GameLogic : BaseJiliSlotGame
    {
        ...
        protected override BaseJiliSlotSpinResult calculateResult(BaseJiliSlotBetInfo betInfo, string strSpinResponse, bool isFirst, JiliFreeSpinInfo freeSpinInfo, double userBalance = 0, double winMoney = 0)
        {
            try
            {
                ...
                var spinResultJson = JsonConvert.DeserializeObject<Fg3SpinClass>(dataSpin["allPlate"]?.ToString(Formatting.None));
                ...
                {
                    foreach (AwardDataFG3 item in spinResultJson.AwardDataVec)
                    {
                        item.Win = getRealWinByBet(item.Win, betInfo.TotalBet);
                    }
                }

                if (spinResultJson.PreAwardDataVec != null && spinResultJson.PreAwardDataVec.Count > 0)
                {
                    foreach (AwardDataFG3 item in spinResultJson.AwardDataVec)
                    {
                        item.Win = getRealWinByBet(item.Win, betInfo.TotalBet);
