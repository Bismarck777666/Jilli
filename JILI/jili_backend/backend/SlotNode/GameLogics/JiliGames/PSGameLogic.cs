...
{
    class PSGameLogic : BaseJiliSlotGame
    {
        ...
        protected override BaseJiliSlotSpinResult calculateResult(BaseJiliSlotBetInfo betInfo, string strSpinResponse, bool isFirst, JiliFreeSpinInfo freeSpinInfo, double userBalance = 0, double winMoney = 0)
        {
            try
            {
                ...
                var spinResultJson = JsonConvert.DeserializeObject<PSSpinClass>(strSpinResponse);
                ...
                foreach (RoundInfoPS item in spinResultJson.RoundQueue)
                {
                    ...
                    if (item.ComboStageData != null && item.ComboStageData.Count > 0)
                    {
                        foreach (ComboStageInfoPS comboItem in item.ComboStageData)
                        {
                            ...
                            {
                                foreach (AwardDataPS awardItem in comboItem.AwardDataVec)
                                {
